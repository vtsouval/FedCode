...
class Block(torch.nn.Module):
	...
	def __init__(self, in_planes, out_planes, expansion, stride):
		...
		if stride == 1 and in_planes != out_planes:
			self.shortcut = torch.nn.Sequential(torch.nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0, bias=False), torch.nn.BatchNorm2d(out_planes),)
...
class MobileNetV2(torch.nn.Module):
	...
	cfg = [
		(1,  16, 1, 1),
		(6,  24, 2, 1), # NOTE: change pooling kernel_size 2 -> 1 for CIFAR10
		(6,  32, 3, 2),
		(6,  64, 4, 2),
		(6,  96, 3, 1),
		(6, 160, 3, 2),
	...
	def forward(self, x):
		...
		out = self.layers(out)
		out = torch.nn.functional.relu(self.bn2(self.conv2(out)))
		out = torch.nn.functional.avg_pool2d(out, 4) # NOTE: change pooling kernel_size 7 -> 4 for CIFAR10
